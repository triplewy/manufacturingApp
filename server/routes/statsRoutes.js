module.exports = function(conn, loggedIn) {
    ...
    statsRoutes.get('/totalDowntime/:timePeriod', loggedIn, (req, res) => {
      ...
      conn.query('SELECT SUM(b.downtime) AS totalDowntime FROM downtime AS b WHERE b.lineId = (SELECT lineId FROM assemblyLines WHERE userId = :userId ORDER BY lineId ASC LIMIT 1) ' +
    ...
    statsRoutes.get('/downtime/machines/:timePeriod', loggedIn, (req, res) => {
      ...
      'WHERE a.lineId = (SELECT lineId FROM assemblyLines WHERE userId = :userId ORDER BY lineId ASC LIMIT 1) GROUP BY a.machineId ORDER BY totalDowntime DESC', {userId: userId}, function(err, result) {
        if (err) {
          console.log(err);
        } else {
          res.send(result)
    ...
    function parseTimePeriod(timePeriod) {
      switch (timePeriod) {
        case 0:
          return 'SELECT SUM(downtime) AS totalDowntime, HOUR(createdDate) AS time FROM downtime WHERE createdDate >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 DAY) AND lineId = (SELECT lineId FROM assemblyLines WHERE userId = :userId ORDER BY lineId ASC LIMIT 1) GROUP BY HOUR(createdDate) ORDER BY time ASC'
        case 1:
          return 'SELECT SUM(downtime) AS totalDowntime, DATE(createdDate) AS time FROM downtime WHERE createdDate >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 WEEK) AND lineId = (SELECT lineId FROM assemblyLines WHERE userId = :userId ORDER BY lineId ASC LIMIT 1) GROUP BY DATE(createdDate) ORDER BY time ASC'
        case 2:
          return 'SELECT SUM(downtime) AS totalDowntime, DATE(createdDate) AS time FROM downtime WHERE createdDate >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 30 DAY) AND lineId = (SELECT lineId FROM assemblyLines WHERE userId = :userId ORDER BY lineId ASC LIMIT 1) GROUP BY DATE(createdDate) ORDER BY time ASC'
        case 3:
          return 'SELECT SUM(downtime) AS totalDowntime, MONTH(createdDate) AS time FROM downtime WHERE createdDate >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 YEAR) AND lineId = (SELECT lineId FROM assemblyLines WHERE userId = :userId ORDER BY lineId ASC LIMIT 1) GROUP BY MONTH(createdDate) ORDER BY time ASC'
        case 4:
          return 'SELECT SUM(downtime) AS totalDowntime, YEAR(createdDate) AS time FROM downtime WHERE createdDate >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 2 YEAR) AND lineId = (SELECT lineId FROM assemblyLines WHERE userId = :userId ORDER BY lineId ASC LIMIT 1) GROUP BY YEAR(createdDate) ORDER BY time ASC'
        default:
          return 'SELECT SUM(downtime) AS totalDowntime, DATE(createdDate) AS time FROM downtime WHERE createdDate >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 WEEK) AND lineId = (SELECT lineId FROM assemblyLines WHERE userId = :userId ORDER BY lineId ASC LIMIT 1) GROUP BY DATE(createdDate) ORDER BY time ASC'
      }
    }
    ...
    function parseTimePeriodLine(timePeriod) {
      switch (timePeriod) {
        case 0:
          return 'SELECT SUM(downtime) AS totalDowntime, HOUR(createdDate) AS time FROM downtime WHERE createdDate >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 DAY) AND lineId = :lineId GROUP BY HOUR(createdDate) ORDER BY time ASC'
        case 1:
          return 'SELECT SUM(downtime) AS totalDowntime, DATE(createdDate) AS time FROM downtime WHERE createdDate >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 WEEK) AND lineId = :lineId GROUP BY DATE(createdDate) ORDER BY time ASC'
        case 2:
          return 'SELECT SUM(downtime) AS totalDowntime, DATE(createdDate) AS time FROM downtime WHERE createdDate >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 30 DAY) AND lineId = :lineId GROUP BY DATE(createdDate) ORDER BY time ASC'
        case 3:
          return 'SELECT SUM(downtime) AS totalDowntime, MONTH(createdDate) AS time FROM downtime WHERE createdDate >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 YEAR) AND lineId = :lineId GROUP BY MONTH(createdDate) ORDER BY time ASC'
        case 4:
          return 'SELECT SUM(downtime) AS totalDowntime, YEAR(createdDate) AS time FROM downtime WHERE createdDate >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 2 YEAR) AND lineId = :lineId GROUP BY YEAR(createdDate) ORDER BY time ASC'
        default:
          return 'SELECT SUM(downtime) AS totalDowntime, DATE(createdDate) AS time FROM downtime WHERE createdDate >= DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 WEEK) AND lineId = :lineId GROUP BY DATE(createdDate) ORDER BY time ASC'
      }
    }
